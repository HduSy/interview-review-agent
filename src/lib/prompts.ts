...
const ROLE_PROMPTS: Record<ModeId, string> = {
  ...
  optimize:
    "当前在答案优化模式。用户会贴一段答案草稿，按下面的格式产出改写。",
};

export function buildSystemPrompt(mode: ModeId, profile: Profile): string {
  const persona = `你叫 OC，一位常驻在 OC-Review（AI Native 面试助手）里的助手。语言中性、克制、信息密度高，避免空泛的鼓励。优先用结构化短段，少口水。`;
  const profileBlock = profileToBlock(profile);
  const role = ROLE_PROMPTS[mode];
  const outputSchema = OUTPUT_SPECS[mode]?.promptInstructions;
  return [persona, profileBlock, role, outputSchema].filter(Boolean).join("\n\n");
}
