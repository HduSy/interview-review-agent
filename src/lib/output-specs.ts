...
function parseOptimize(text: string): ParseResult | null {
  ...
  return {
    ...
    summary: extractSection(text, "说明", "Note") ?? "改写完了 — 下面是 Before / After 对照。",
  };
}
...
function parsePartialOptimize(text: string): ParseResult | null {
  ...
  return {
    ...
    summary: extractSection(text, "说明", "Note") ?? "正在改写…",
  };
}
...
const REVIEW_PREAMBLE = "拿到了，我从 STAR、技术深度、表达三个维度跑了一遍。";
...
题目段格式：每行一题，序号 + . + 空格 + 题干 + " | " + 类型 + " | " + 0-100 整数（不要 %）。生成 8-12 题，**按概率从高到低排列**。类型例如：前端框架 / 系统设计 / 行为面 / 算法 / 工程化 / TypeScript / 性能等。`;

const PREDICT_PREAMBLE = "基于你提供的目标 + 简历，我抽出了高概率题。";
...
[评分维度]
（4 个维度，用 · 分隔，例如「可扩展性 · 性能直觉 · 取舍说明 · 边界情况」）

各段之间空一行。`;

const PRACTICE_PREAMBLE = "随机抽一道题，90 秒读题，然后开始作答。";
